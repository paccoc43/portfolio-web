...
function moveBall(s) {
  ...
  const hitsPlayerPad = s.bx - BALL < PAD_W + 6 && s.by > s.py && s.by < s.py + PAD_H && s.vx < 0;
  if (hitsPlayerPad) {
    s.vx = Math.abs(s.vx) * 1.04;
    s.vy += ((s.by - (s.py + PAD_H / 2)) / (PAD_H / 2)) * 3.4;
  }
  const hitsAiPad = s.bx + BALL > W - PAD_W - 6 && s.by > s.ay && s.by < s.ay + PAD_H && s.vx > 0;
  if (hitsAiPad) {
    s.vx = -Math.abs(s.vx) * 1.04;
...
// Devuelve null si nadie anotó, o { over, message } cuando hay punto
// (message solo se rellena si la partida termina con ese punto).
function registerScore(s) {
  let scored = false;
  ...
  const over = s.p >= WIN || s.ai >= WIN;
  let message = null;
  if (over) message = s.p >= WIN ? "¡Ganaste!" : "Gana la IA";
  return { over, message };
}

function drawScene(ctx, s, colors) {
  const { accent, text, faint, bg } = colors;
...
function readThemeColors() {
  return {
    accent: pfVar("--accent", "#5bbf7a"),
    text: pfVar("--text", "#e0e0e0"),
    faint: pfVar("--border", "#333"),
    bg: pfVar("--bg", "#0a0a0a"),
  };
}
...
function PongGame() {
  ...
  useEffect(() => {
    ...
    const step = () => {
      ...
      movePaddleByKeys(s, keysRef.current);
      moveAiPaddle(s);
      moveBall(s);

      const result = registerScore(s);
      if (result) {
      ...
      drawScene(ctx, s, readThemeColors());
  ...
  let statusLabel = "Tú · IA";
  if (winner) statusLabel = winner;
  else if (running) statusLabel = "Tú · IA — a 5 puntos";

  let playLabel = "▶ Jugar";
  if (winner) playLabel = "Jugar otra vez";
  else if (score.p || score.ai) playLabel = "Seguir";
    ...
    <div>
      <div className="pf-stage-bar">
        <div style={{ display: "flex", alignItems: "center", gap: 18 }}>
          ...
          <span className="pf-stage-label">{statusLabel}</span>
        </div>
        <div style={{ display: "flex", gap: 8 }}>
          {!running && (
            <button type="button" className="pf-game-btn" onClick={() => { if (winner) { init(); } setRunning(true); }}>
              {playLabel}
            </button>
          )}
          {running && <button type="button" className="pf-game-btn sec" onClick={() => setRunning(false)}>Pausa</button>}
          <button type="button" className="pf-game-btn sec" onClick={() => { setRunning(false); init(); }}>Reiniciar</button>
        </div>
      </div>
